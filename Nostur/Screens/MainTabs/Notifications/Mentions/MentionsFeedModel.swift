    ...
    public func load(limit: Int?, includeSpam: Bool = false, completion: ((Int64) -> Void)? = nil) {
        ...
        bgContext.perform { [weak self] in
            ...
            r1.predicate = NSPredicate(
                format: "NOT pubkey IN %@ AND kind IN {1,1111,1222,1244,20,9802,30023,34235,1311} AND tagsSerialized CONTAINS %@ AND NOT id IN %@ AND (replyToRootId == nil OR NOT replyToRootId IN %@) AND (replyToId == nil OR NOT replyToId IN %@) AND flags != \"is_update\" ",
                (AppState.shared.bgAppState.blockedPubkeys + [pubkey] + Array(AnonReplySession.shared.bgAnonPubkeys)),
                serializedP(pubkey),
    ...
    public func showMore() {
        ...
        bg().perform { [weak self] in
            ...
            if let until = allMentionEvents.last?.created_at {
                req(RM.getMentions(
                    pubkeys: [pubkey],
                    kinds: [1,1111,1222,1244,20,9802,30023,34235,1311],
                    limit: 500,
