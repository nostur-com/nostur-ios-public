...
extension Search {
    ...
    func nprofileSearch(_ term: String, searchID: UUID) {
    ...
    func naddrSearch(_ term: String, searchID: UUID) {
    ...
    func npubSearch(_ term: String, searchID: UUID) {
    ...
    func nametagSearch(_ term: String, searchID: UUID) {
    ...
    func hashtagSearch(_ term: String, searchID: UUID) {
        let blockedPubkeys = blocks()
        searching = true
        contacts = []
        nrPosts = []
        ...
        let searchTask1 = ReqTask(prefix: "SEA-", reqCommand: { taskId in
            ...
            guard self.searchID == searchID else { return }
            let existingIds = self.nrPosts.map(\.id)
            bg().perform {
                let fr = Event.fetchRequest()
                fr.predicate = NSPredicate(format: "kind IN {1,1111,1222,1244,20,9802} AND NOT pubkey IN %@ AND tagsSerialized CONTAINS[cd] %@", blockedPubkeys, serializedT(term))
                fr.fetchLimit = 150
    ...
    func note1Search(_ term: String, searchID: UUID) {
    ...
    func hexIdSearch(_ term: String, searchID: UUID) {
    ...
    func otherSearch(_ term: String, searchID: UUID) {
        searching = true
    ...
    func nip05Search(_ nip05parts:Nip05Parts, searchID: UUID) {
        ...
        searchTask = Task {
            do {
                ...
                guard !Task.isCancelled, self.searchID == searchID else { return }
                guard let nostrJson = try? JSONDecoder().decode(NostrJson.self, from: data),
                      let pubkey = nostrJson.names[nip05parts.name],
                      !pubkey.isEmpty
                else {
                    searching = false
                    return
                }
                hexIdSearch(pubkey, searchID: searchID)
            }
            catch {
                guard !Task.isCancelled, self.searchID == searchID else { return }
                searching = false
    ...
    func urlSearch(_ term: String, searchID: UUID) {
        let blockedPubkeys = blocks()
        searching = true
        contacts = []
