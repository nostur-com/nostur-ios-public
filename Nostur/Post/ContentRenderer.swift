...
struct ContentRenderer: View { // VIEW things
    ...
    init(nrPost: NRPost, showMore: Binding<Bool>, isDetail: Bool = false, fullWidth: Bool = false, forceAutoload: Bool = false, hideImages: Bool = false, zoomableId: String = "Default") {
        ...
        // Row only: collapse consecutive images into adaptive grids. Detail keeps stacked images.
        let baseElements = isDetail ? nrPost.contentElementsDetail : nrPost.contentElements
        _contentElements = State(wrappedValue: isDetail ? baseElements : collapseImageGrids(baseElements))
    ...
    //                                        .centered()
        ...
        .onChange(of: showMore) { [oldValue = self.showMore] newValue in
            if newValue && !oldValue {
                withAnimation {
                    // Keep adaptive grids on rows when expanding text; detail uses the full image list
                    let detail = self.nrPost.contentElementsDetail
                    self.contentElements = self.isDetail ? detail : collapseImageGrids(detail)
                }
            }
        }
