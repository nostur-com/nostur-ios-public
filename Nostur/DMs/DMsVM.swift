...
class DMsVM: ObservableObject, Equatable, Hashable {
    ...
    {
        didSet {
            let acceptedPubkeys = Set(dmStates.filter { $0.accepted }.compactMap { $0.contactPubkey_ })
            allowedWoTLock.lock()
            allowedWoT = acceptedPubkeys
            allowedWoTLock.unlock()
        }
    }

    // pubkeys we started a conv with (but maybe not in WoT), should be allowed in DM WoT
    // Add this to WoT
    private var allowedWoT: Set<String> = []
    private let allowedWoTLock = NSLock()

    public func isAllowedByWoT(_ pubkey: String) -> Bool {
        allowedWoTLock.lock()
        let isAllowed = allowedWoT.contains(pubkey)
        allowedWoTLock.unlock()
        return isAllowed
    }
