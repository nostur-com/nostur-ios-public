...
class ProfileZapsViewModel: ObservableObject {
    ...
    @Published var posts: [NRPost] = [] {
        didSet {
            ...
            L.og.info("Profile Zaps: loaded \(self.posts.count) posts")
        }
    }
    ...
    private func fetchZapsFromRelays(_ onComplete: (() -> ())? = nil) {
        let reqTask = ReqTask(
                ...
                if let cm = NostrEssentials
                            .ClientMessage(type: .REQ,
                                           ...
                                           filters: [
                                            Filters(
                                                kinds: Set([9735]),
                                                tagFilter: TagFilter(tag: "P", values: [self.pubkey]),
                                                limit: 500
                                            )
                                           ]
                            ).json() {
                    ...
                    L.og.error("Profile Zaps: Problem generating request")
                }
            },
            processResponseCommand: { [weak self] taskId, relayMessage, _ in
                ...
                L.og.info("Profile Zaps: ready to process relay response")
            },
            timeoutCommand: { [weak self] taskId in
                self?.backlog.clear()
                self?.fetchZapsFromDB(onComplete)
                L.og.info("Profile Zaps: timeout ")
            })
    ...
    private func fetchZapsFromDB(_ onComplete: (() -> ())? = nil) {
        ...
        fr.predicate = NSPredicate(format: "kind == 9735 AND zapFromRequest.pubkey == %@", self.pubkey)
    ...
    private func fetchPostsFromRelays(_ onComplete: (() -> ())? = nil) {
        ...
        bg().perform { [weak self] in
            ...
            guard !onlyNewIds.isEmpty else {
                L.og.debug("Profile Zaps: fetchPostsFromRelays: empty ids")
                if (self.zappedEventIds.count > 0) {
                    L.og.debug("Profile Zaps: but we can render the duplicates")
            ...
            L.og.debug("Profile Zaps: fetching \(self.zappedEventIds.count) posts, skipped \(self.zappedEventIds.count - onlyNewIds.count) duplicates")
            
            let reqTask = ReqTask(
                debounceTime: 0.5,
                subscriptionId: "PROFILE-ZAPPED-POSTS",
                reqCommand: { taskId in
                    if let cm = NostrEssentials
                        ...
                        L.og.error("Profile Zaps: Problem generating posts request")
                    }
                },
                processResponseCommand: { [weak self] taskId, relayMessage, _ in
                    self?.fetchPostsFromDB(onComplete)
