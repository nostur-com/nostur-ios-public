...
extension Event {
    ...
    func isLive() -> Bool {
        ...
        // so only keep if $0.created_at is newer than 12 hours ago AND we have a "streaming" tag, should be enough sanity check
        // (AND also "live")
        let createdAt = Date(timeIntervalSince1970: Double(self.created_at))
        let eightHoursAgo = Date().addingTimeInterval(-12 * 60 * 60)
