        ...
        let task = ReqTask(
            ...
            processResponseCommand: { [weak self] taskId, relayMessage, event in
                ...
                bg().perform { [weak self] in
                    ...
                    fr.predicate = NSPredicate(format: "created_at >= %i AND pubkey IN %@ AND kind IN %@ AND flags != \"is_update\"", Int(since), self.enabledPubkeys, PROFILE_KINDS.subtracting(Set([6]))) // not reposts
