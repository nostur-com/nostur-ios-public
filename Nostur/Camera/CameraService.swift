...
public class CameraService {
    ...
    public func checkForPermissions() {
        ...
        default:
            ...
            DispatchQueue.main.async {
                self.alertError = AlertError(title: "Camera Access", message: "No access to use your camera, please update your privacy settings.", primaryButtonTitle: "Settings", secondaryButtonTitle: nil, primaryAction: {
    ...
    private func configureSession() {
        ...
        do {
            ...
            guard let videoDevice = defaultVideoDevice else {
                L.og.debug("Default video device is unavailable.")
            ...
            if session.canAddInput(videoDeviceInput) {
                ...
                L.og.debug("Couldn't add video device input to the session.")
                setupResult = .configurationFailed
                session.commitConfiguration()
                return
            }
        } catch {
            L.og.debug("Couldn't create video device input: \(error)")
            setupResult = .configurationFailed
            session.commitConfiguration()
            return
        }
    ...
    public func changeCamera() {
        ...
        sessionQueue.async {
            ...
            @unknown default:
                L.og.debug("Unknown capture position. Defaulting to back, dual-camera.")
                preferredPosition = .back
                preferredDeviceType = .builtInWideAngleCamera
            }
            ...
            if let videoDevice = newVideoDevice {
                do {
                    ...
                    L.og.debug("Error occurred while creating video device input: \(error)")
                }
            }
        ...
        sessionQueue.async {
            if !self.isSessionRunning && self.isConfigured {
                ...
                case .configurationFailed, .notAuthorized:
                    L.og.debug("Application not authorized to use camera")
